(function(){
	...
	*/
	...
	var oSys = {
		...
		PName: [oPeashooter, oSunFlower, oWallNut, oPotatoMine, oSnowPea, oChomper, oRepeater, oPuffShroom, oFumeShroom, oHypnoShroom, oScaredyShroom, oSquash, oThreepeater, oSpikeweed, oTorchwood, oTallNut, oCactus, oSplitPea, oStarfruit, oPumpkinHead, oFlowerPot, oGarlic, oGatlingPea, oGloomShroom, oSpikerock
		],
		ZName: [oZombie],
			...
			oP.Monitor({
				...
				f: function() {
					...
					var b = function(c) {
						...
						NewEle("btnClickSave", "button", "position:absolute;left:750px;top:250px;height:50px;width:100px;font-family:幼圆;font-size:18px;z-index:100", { // 保存按钮
							innerHTML: "Save Level",
							onclick: function() {
								var g = oGd.$, k, m = "", i, l, f, d = oS.ArP.ArC[1] - 1, h = oS.ArP.ArR[1], r = 0, z = "", 
								j = { // 植物数据
									"default": "01", // 默认
								...
								if ($User.Visitor.UserName == "游客") return alert("Only logged in users can save progress！\nPlease log in before saving your progress!"); // 未登录用户

								if ($P.length < h * d * (4 / 5)) return alert("The amount of plants must reach 80% of the set range!\n\nPlease save again after completing the layout！"); // 植物没满

								if (r <= 0 || r >= c || r != c - 1) return alert("The plants exceed the planting range limit or the formation does not meet the specifications.！\n\nPlease adjust the formation and save again！"); // 植物种植在了线外 / 没到指定列
								
								if ((f = prompt("Please enter the amount of sunlight, range 50-1000 and must be multiples of 25 \nFor example：150,175,200,225,250,275,300,325,350", "150")) == null) return; // 用户未输入, 返回
								if (isNaN(f = Number(f)) || f % 25 != 0 || f < 50 || f > 1000) return alert("Please enter a number in the range 50-750 that is a multiple of 25！"); // 输入阳光
								
								if ((l = prompt("Please enter a title for your custom game(within 50 characters)\nIf not entered, the default title will be used.", "")) != null) {
					...
					innerText(NewEle("spanT", "span", "position:absolute;left:65px;width:620px;text-align:left; font-family: 幼圆; font-size: 14px;line-height:50px", 0, a), "How many columns to place your plants in: "); // 选列
	...
	$SEql(IZMode, { // 每个阶段函数
		"ChoseMode": function () { // 选择模式
			oS.Init($FJ(oSys, {
				...
				LoadAccess: function () {
					...
					NewEle("dChoseTitle", "div", "position:relative;text-align:center;line-height:88px;height:88px;width:100%;font-size:30px;font-weight:bold;font-family:黑体;color:#fff", { innerHTML: "Select mode" }, $("dChosePanel"), {"class":"dRiddleTitle"});
					...
					NewEle("dGrassDiv", "div", "left:100px;top:100px;background-image:url(images/interface/background2.jpg);display:block;position:absolute;z-index:100;cursor:pointer;background-position:-25px,0px;background-size:324px,139px;background-repeat:no-repeat;width:275px;height:139px;border:5px solid rgba(255,255,255,0.5);border-radius:15px;", { onclick: function() { Change_Level("NGrass"); } }, EDAll);
					NewEle("dGrassTXT", "div", "text-align:center;line-height:60px;font-size:30px;font-weight:bold;font-family:黑体;color:#fff;position:relative;top:15px;", { innerHTML: "Night<br><font style=\"font-size:20px\">Click here to select this mode</font>" }, $("dGrassDiv"));

					NewEle("dPoolDiv", "div", "left:100px;top:250px;background-image:url(images/interface/background4.jpg);display:block;position:absolute;z-index:100;cursor:pointer;background-position:-25px,0px;background-size:324px,139px;background-repeat:no-repeat;width:275px;height:139px;border:5px solid rgba(255,255,255,0.5);border-radius:15px;", { onclick: function() { Change_Level("NPool"); } }, EDAll);
					NewEle("dPoolTXT", "div", "text-align:center;line-height:60px;font-size:30px;font-weight:bold;font-family:黑体;color:#fff;position:relative;top:15px;", { innerHTML: "Night Pool<br><font style=\"font-size:20px\">Click here to select this mode</font>" }, $("dPoolDiv"));
		...
		"NPool": function () { // 黑夜泳池 NPool
			oS.Init($FJ(oSys, {
				...
				PName: [oPeashooter, oSunFlower, oWallNut, oPotatoMine, oSnowPea, oChomper, oRepeater, oPuffShroom, oFumeShroom, oHypnoShroom, oScaredyShroom, oLilyPad, oSquash, oThreepeater, oTangleKlep, oSpikeweed, oTorchwood, oTallNut, oSeaShroom, oCactus, oSplitPea, oStarfruit, oPumpkinHead, oFlowerPot, oGarlic, oGatlingPea, oGloomShroom, oSpikerock],
