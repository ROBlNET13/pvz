...
//      20: $("imgFlag3"),
          ...
          (i < u.FlagNum
            ...
            : (NewEle(
              ...
              !isNaN(Math.floor(s)) &&
                ...
                (t.childNodes[1].innerHTML = (s = s - Math.floor(s / 10) * 10)
                  ? s
                  : s + 1)),
              PauseGame($("dMenu0"), 1)));
      };
  ...
  },
  {
    ...
    FlagToSumNum: {
      a1: [
        3, 6, 9, 13, 15, 19, 20, 21, 23, 26, 28, 32, 34, 37, 39, 40, 41, 42, 43,
        45, 48, 52, 58, 59,
      ],
      a2: [
        4, 8, 12, 20, 30, 50, 150, 160, 14, 20, 27, 36, 43, 50, 56, 200, 50, 10,
        15, 25, 39, 50, 60, 70, 120,
      ],
    },
    FlagToMonitor: {
      19: [ShowLargeWave, 0],
    ...
    dag_traversal_of: function () {
      for (let _ in $Z) {
        ...
        if (!to && (b.WalkDirection ? redag[now] : dag[now])) {
          if (b.WalkDirection) {
            b.Next_Edge = to =
              redag[now][Math.floor(Math.random() * redag[now].length)];
          } else {
            b.Next_Edge = to =
              dag[now][Math.floor(Math.random() * dag[now].length)];
          }
        }
        if (!to) {
