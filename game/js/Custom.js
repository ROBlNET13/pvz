...
(function () {
	...
	var scaleEnabled = params.get("scale") === "true";
	var isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) || screen.width <= 1024;

	if (scaleEnabled && isMobile) {
		function fitWindowToViewport() {
			...
			var scaleX = viewportWidth / windowWidth;
			var scaleY = viewportHeight / windowHeight;
			...
			// Center the content
			var scaledWidth = windowWidth * scale;
			var scaledHeight = windowHeight * scale;
			var marginLeft = Math.max(0, (viewportWidth - scaledWidth) / 2) / scale;
			var marginTop = Math.max(0, (viewportHeight - scaledHeight) / 2) / scale;
